 ...
 */
public class HighestPriceProcessor extends AbstractFollowedDataProcessor {
    ...
    public List<ContentLabel> getFollowedDatas(DailyTradeData dtd, Map<String, Object> param) {
        ...
        for (DailyTradeData dailyTradeData : dailyTradeDatas) {
            if (highestPrice < dailyTradeData.getHighestPrice()) {
                highestDate = dailyTradeData.getCurrentDate();
                highestPrice = dailyTradeData.getHighestPrice();
            }
        }
        double rate = (dtd.getClosingPrice() - highestPrice) / highestPrice;
